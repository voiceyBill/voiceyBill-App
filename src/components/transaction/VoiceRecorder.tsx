  ...
  const convertWavToMp3 = async (wavUri: string): Promise<string> => {
    ...
    for (let i = 0; i < mp3Buffer.length; i += chunk)
      bin += String.fromCharCode.apply(
        null,
        Array.from(
          mp3Buffer.subarray(i, Math.min(i + chunk, mp3Buffer.length)),
        ),
      );
    const mp3Uri =
      FileSystem.cacheDirectory + "recording_" + Date.now() + ".mp3";
    ...
    <View style={styles.container}>
          ...
          style={[
            styles.resultCard,
            {
              borderColor: isIncome
                ? themeColors.incomeBg
                : themeColors.expenseBg,
        ...
        >
          ...
          >
            <View style={styles.resultBarLeft}>
              {isIncome ? (
                <ArrowUpRight
                  size={15}
                  color={themeColors.incomeText}
                  strokeWidth={2.5}
                />
              ) : (
                <ArrowDownRight
                  size={15}
                  color={themeColors.expenseText}
                  strokeWidth={2.5}
                />
              )}
              <Text
          ...
          <View style={styles.resultBody}>
            ...
            <View style={styles.resultTitleRow}>
              <Text
                style={[styles.resultTitle, { color: themeColors.foreground }]}
                numberOfLines={2}
              >
              ...
              >
                {transcriptionResult.amount != null
                  ? formatCurrency(transcriptionResult.amount, {
                      currency: transcriptionResult.currency || baseCurrency,
                      showSign: true,
                      isExpense: !isIncome,
                    })
                  : "—"}
                ...
                >
                  ...
                  >
                    {(() => {
                      try {
                        return format(
                          new Date(transcriptionResult.date),
                          "MMM d, yyyy",
                        );
                      } catch {
                        return transcriptionResult.date;
                      }
                    })()}
                  </Text>
                </View>
      ...
      {!transcriptionResult && (
        <Text style={[styles.helpText, { color: themeColors.mutedForeground }]}>
          Tip: "I spent $1500 at the supermarket for groceries today using my
          credit card"
        </Text>
      )}
    </View>
